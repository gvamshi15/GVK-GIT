...
<xsl:stylesheet version="1.0"
  ...
  xmlns:java="http://xml.apache.org/xslt/java" exclude-result-prefixes="java">
	...
	<xsl:template name="pageDefinitions">
		...
		<fo:layout-master-set>
			<fo:simple-page-master master-name="A4-portrait"
				...
				margin-bottom="0.5cm" margin-left="2cm" margin-right="1cm">				
					...
					<fo:table-row>
						<fo:table-cell column-number="1">
							...
							<fo:block text-align="center" font-weight="bold" font-size="12pt" text-decoration="underline">
								<xsl:value-of select="it-print-labels/txt-shipment"/><xsl:text>&#160;</xsl:text>
								<xsl:value-of select="shipment-no"/> 
							</fo:block>
						</fo:table-cell>
					</fo:table-row>
				</fo:table-body>
	...
	<xsl:template match="it-print-head-dOM">	
		<fo:root hyphenate="true" font-family="Cyrillic">
			...
			<fo:page-sequence master-reference="A4-portrait">
				<xsl:call-template name="headerAndFooter" />
					<fo:flow flow-name="xsl-region-body">
				            ...
				            <fo:table-header>
				              <fo:table-row text-align="center" font-size="9pt" font-weight="bold">
				                ...
				                <fo:table-cell column-number="6">
				                  ...
				                  padding-before="0.15cm" border-top-style="solid" 
				                  border-width="1pt" border-color="black">
				               		<xsl:value-of select="it-print-labels/txt-posted" />
				               	  </fo:block>	
				                </fo:table-cell>
				              </fo:table-row>
				              <fo:table-row text-align="center" font-size="9pt" font-weight="bold">
				          	...
				          	<fo:table-body>				          	
				          		<xsl:for-each select="it-print-positions">				          		
					                  ...
					                  <fo:table-cell column-number="6" text-align="left" padding-before="0.15cm">
					                    <fo:block>
					                      <xsl:value-of select="qty-posted-by-wMDE" />
					                    </fo:block>
					                  </fo:table-cell>						                  
					                </fo:table-row>				          		
				          		</xsl:for-each>				          		
						...
						<fo:table width="100%" table-layout="fixed" padding-before="0.50cm">
							<fo:table-column column-width="proportional-column-width(1)" column-number="1"/>
							<fo:table-body>
								<fo:table-row>
									<fo:table-cell column-number="1">
								...
								<fo:table-row>
									<fo:table-cell column-number="1">
										<fo:block font-size="9pt">
											<xsl:value-of select="it-print-labels/txt-outbound" /><xsl:text>&#160;</xsl:text> 
											<xsl:value-of select="outbound-date" />
										</fo:block>
										<fo:block font-size="9pt">
											<xsl:value-of select="it-print-labels/txt-done-by" /><xsl:text>&#160;</xsl:text>
											<xsl:value-of select="outbound-user" />
										</fo:block>
										<fo:block font-size="9pt" text-decoration="underline" padding-before="0.25cm">
											<xsl:value-of select="it-print-labels/txt-signature" /> 
										</fo:block>
										<fo:block font-size="9pt">																						
											<xsl:variable name="outBoundSignatureFile">
												<xsl:value-of select="full-path-of-outbound-signature-file" />
											</xsl:variable>											
											<fo:external-graphic src="{$outBoundSignatureFile}" />											
										</fo:block>
									</fo:table-cell>
									<fo:table-cell column-number="2">
										<fo:block font-size="9pt">
											<xsl:value-of select="it-print-labels/txt-inbound"/><xsl:text>&#160;</xsl:text> 
											<xsl:value-of select="inbound-date"/>
										</fo:block>
										<fo:block font-size="9pt">
											<xsl:value-of select="it-print-labels/txt-done-by"/><xsl:text>&#160;</xsl:text> 
											<xsl:value-of select="inbound-user" />
										</fo:block>
										<fo:block font-size="9pt" text-decoration="underline" padding-before="0.25cm">
											<xsl:value-of select="it-print-labels/txt-signature" /> 
										</fo:block>
										<fo:block>																						
											<xsl:variable name="inBoundSignatureFile">
												<xsl:value-of select="full-path-of-inbound-signature-file" />
											</xsl:variable>											
											<fo:external-graphic src="{$inBoundSignatureFile}" />											
										</fo:block>
									</fo:table-cell>
								</fo:table-row>
							</fo:table-body>
